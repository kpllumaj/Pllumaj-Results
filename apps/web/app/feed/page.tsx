...
function FeedContent() {
  ...
  const activeNeed = useMemo(() => needs.find((need) => need.id === activeNeedId) ?? null, [needs, activeNeedId]);
  ...
  const activeNeedChannel = useMemo(() => (activeNeedId ? `need-${activeNeedId}` : null), [activeNeedId]);
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-14">
      <header className="mb-12 text-center">
        ...
        <p className="mt-3 text-base text-gray-600">Real-time requests from clients in Pllumaj Results.</p>
      </header>
            ...
            >
              <div className="flex-grow space-y-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-emerald-700">
                    {need.budgetAmount != null ? `${need.budgetCurrency || "USD"} ${Number(need.budgetAmount).toFixed(2)}` : "Budget TBD"}
                  </span>
                </div>
              ...
              <div className="mt-6 flex items-center justify-between gap-4 text-sm text-gray-500">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-400">Client: {need.client?.email ?? "Hidden"}</p>
                </div>
      ...
      {toastMessage ? (
        <div
          className={`fixed right-4 top-24 z-50 rounded-lg px-4 py-3 text-sm font-semibold text-white shadow-lg ${toastMessage.tone === "success" ? "bg-emerald-600" : "bg-red-600"}`}
          role="status"
          aria-live="polite"
        >
      ...
      {activeNeed ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center px-4" style={{ backgroundColor: "rgba(0, 0, 0, 0.55)" }}>
          <div className="w-full max-w-md p-6 shadow-2xl" style={{ backgroundColor: "#ffffff", borderRadius: "16px" }}>
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-xl font-semibold text-gray-900">Send Offer</h2>
                <p className="text-sm text-gray-500">Respond to &ldquo;{activeNeed.title}&rdquo;</p>
              </div>
              <button
                type="button"
            ...
            <form className="space-y-4" onSubmit={handleOfferSubmit}>
              ...
              <label className="block text-sm font-medium text-gray-700">
                Currency
                <select value={offerCurrency} onChange={(event) => setOfferCurrency(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 p-2">
